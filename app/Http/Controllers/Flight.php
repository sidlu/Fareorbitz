...
{ 	
		...
		//live API 2nd
		/*public $client_secret='AzsUmzVRrHl4DPA8';
		public $ClientId='hJmx0PEl0i468nsJaY1xATOGxUWVoZCv';*/
		
		//live API 3nd
		public $client_secret='SBwSZpxBX1rR06lA';
		public $ClientId='Ys9GvYCbLH29hdBVFXUMYfrrmBVwxguk';
		...
		public function createToken(){
			...
			$headers= array('Content-Type: application/x-www-form-urlencoded');  //'Content-Type: application/json','Accept: application/json'
			$ch = curl_init();
			...
			curl_close($ch);
			$this->createLogFile('Create_token',$URL,json_encode($headers),json_encode($postdata),$contents);
    ...
    {		
		...
		$this->createLogFile('FlightLists',$URL,json_encode($headers),$URL,$contents);
		$countryFrom='';
		$countryTo='';
   ...
   echo json_encode($data);
	...
	public function SelectFlight(Request $request) {
		...
		$total_responseArr[]=json_decode($total_response,true);
			...
			$this->createLogFile('check-price',$actionUrl,json_encode($headers),json_encode($postdata),$contents);
	...
	public function BookFlight(Request $request){	
	   ...
	   if($payment_status=='Confirmed'){
			...
			//$headers= array('Content-Type: application/vnd.amadeus+json', 'Authorization: Bearer '.$TokenId); 
			//$headers=array('Content-Type: application/json','Accept: application/json','Authorization: Bearer '.$TokenId);
			echo "<br>order_id==".$request->input('order_id');  echo "<br>";
			echo "<br>book_request==".$book_request;  echo "<br>";
	...
	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	curl_setopt($ch, CURLOPT_ENCODING, "gzip");
    echo "<br><br><br>Response==".$contents = curl_exec($ch);
	curl_close($ch);
			
			//$this->createLogFile('book-flight',$actionUrl.'\r\nHeader='.json_encode($headers).'\r\nRequest='.$book_request,$contents);
			$this->createLogFile('book-flight',$actionUrl,json_encode($headers),$book_request,$contents);
			$Response=json_decode($contents,true);
			die;
	...
	$to=$this->currency;
		...
		public function createLogFile($action,$url,$headers,$request,$response){ 
